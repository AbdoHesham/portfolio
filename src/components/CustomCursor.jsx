...
const CustomCursor = () => {
  ...
  return (
    <>
      ...
      <div
        ref={spotlightRef}
        className="fixed top-0 left-0 w-[700px] h-[700px] rounded-full pointer-events-none z-[9998] opacity-0 blur-[100px] transition-opacity duration-300 hidden lg:block"
        style={{
          background: 'radial-gradient(circle, rgba(0,240,255,0.2) 0%, rgba(0,240,255,0.06) 45%, transparent 75%)'
        }}
      ></div>
      ...
      <div
        ref={dotRef}
        className="fixed top-0 left-0 z-[9999] pointer-events-none w-3 h-3 bg-[#00f0ff] rounded-full shadow-[0_0_15px_#00f0ff] hidden lg:block"
      ></div>

      {/* Global Custom Cursor Ring */}
      <div
        ref={ringRef}
        className="fixed top-0 left-0 z-[9999] pointer-events-none w-12 h-12 border border-[#00f0ff]/60 rounded-full flex items-center justify-center backdrop-blur-[1px] hidden lg:block"
      ></div>
    </>
  );
};
