...
const Contact = () => {
  ...
  return (
    <section ref={ref} id="contact" className="bg-[#0b0b0b] w-full min-h-screen relative overflow-hidden flex items-end pt-20 md:pt-32 pb-0 border-t border-white/10 select-none">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] max-w-full h-[600px] bg-[#00f0ff]/15 rounded-full blur-[160px] pointer-events-none z-0"></div>
      ...
      >
        <h1 
          className="text-[25vw] leading-[0.75] font-black text-[#00f0ff] uppercase tracking-tighter select-none scale-y-[1.6] origin-top"
      ...
      <div className="relative z-10 w-full flex justify-end items-end">
        <motion.div 
          ...
          className="bg-[#141414]/95 backdrop-blur-2xl border-t border-l border-white/15 w-full md:w-[90%] lg:w-[82%] p-5 sm:p-8 md:p-16 text-white flex flex-col justify-between rounded-tl-[2rem] sm:rounded-tl-[3rem] shadow-[0_-25px_60px_rgba(0,0,0,0.9)] relative overflow-hidden"
        >
          {/* Subtle internal top cyan highlight glow */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-1 bg-gradient-to-r from-transparent via-[#00f0ff] to-transparent opacity-90"></div>

          <div className="flex flex-col gap-5 sm:gap-6 mb-8 md:mb-14">
            <div className="flex items-center justify-between">
              <div className="inline-flex items-center gap-2 px-3 sm:px-3.5 py-1.5 rounded bg-[#00f0ff]/10 border border-[#00f0ff]/30 text-[10px] sm:text-xs font-mono uppercase tracking-wider sm:tracking-widest text-[#00f0ff]">
              ...
              <div className="flex-1 flex flex-col gap-6 sm:gap-8">
                <div className="relative pb-2">
                  <input 
                    ...
                    className={`w-full bg-transparent border-b pb-3 text-base sm:text-lg focus:outline-none transition-colors placeholder-white/40 font-medium rounded-none text-white ${
                      errors.firstName ? 'border-red-500/70 focus:border-red-400' : 'border-white/20 focus:border-[#00f0ff]'
                    }`}
                  />
                  {errors.firstName && (
                ...
                <div className="relative pb-2">
                  <input 
                    ...
                    className={`w-full bg-transparent border-b pb-3 text-base sm:text-lg focus:outline-none transition-colors placeholder-white/40 font-medium rounded-none text-white ${
                      errors.lastName ? 'border-red-500/70 focus:border-red-400' : 'border-white/20 focus:border-[#00f0ff]'
                    }`}
                  />
                  {errors.lastName && (
                    <span className="absolute bottom-[-10px] left-0 text-[10px] font-mono text-red-400">
                ...
                <div className="relative pb-2">
                  <input 
                    ...
                    className={`w-full bg-transparent border-b pb-3 text-base sm:text-lg focus:outline-none transition-colors placeholder-white/40 font-medium rounded-none text-white ${
                      errors.email ? 'border-red-500/70 focus:border-red-400' : 'border-white/20 focus:border-[#00f0ff]'
                    }`}
                  />
                  {errors.email && (
                    <span className="absolute bottom-[-10px] left-0 text-[10px] font-mono text-red-400">
                ...
                <div className="relative h-full flex flex-col pb-2">
                  <textarea 
                    ...
                    className={`w-full h-full min-h-[120px] sm:min-h-[140px] bg-transparent border-b pb-3 text-base sm:text-lg focus:outline-none transition-colors placeholder-white/40 font-medium resize-none rounded-none text-white ${
                      errors.message ? 'border-red-500/70 focus:border-red-400' : 'border-white/20 focus:border-[#00f0ff]'
                    }`}
                  ></textarea>
                  {errors.message && (
                    <span className="absolute bottom-[-10px] left-0 text-[10px] font-mono text-red-400">
            ...
            <AnimatePresence>
              {status === 'success' && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -8 }}
                  className="p-4 rounded-xl bg-[#00f0ff]/10 border border-[#00f0ff]/40 text-[#00f0ff] font-mono text-xs flex items-center gap-3 shadow-[0_0_20px_rgba(0,240,255,0.15)]"
                >
              ...
              <div className="flex-1 flex flex-col gap-1 text-xs sm:text-sm font-light text-white/70">
                <div className="flex items-start gap-3 sm:gap-4">
                  <input 
                    ...
                    onChange={handleChange}
                    className="mt-0.5 w-4 h-4 rounded-sm border-white/30 bg-transparent text-[#00f0ff] focus:ring-0 focus:ring-offset-0 cursor-pointer shrink-0" 
              ...
              <div className="flex-1 flex flex-col gap-6 sm:gap-8 text-xs text-white/50 font-light">
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-5 sm:gap-6">
                  ...
                  <button 
                    ...
                    className={`w-full sm:w-auto px-8 py-3.5 rounded bg-[#00f0ff] text-black font-bold uppercase tracking-widest text-xs flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(0,240,255,0.6)] whitespace-nowrap transition-all duration-300 ${
                      status === 'loading'
                        ? 'opacity-70 cursor-wait'
                        : 'hover:bg-[#38bdf8] hover:scale-105 active:scale-95 cursor-pointer group'
                    }`}
                  >
                    {status === 'loading' ? (
                      <>
